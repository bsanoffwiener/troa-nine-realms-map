...
const Stars: React.FC = (props) => {
    ...
    const [geo, mat, coords] = useMemo(() => {
        ...
        const mat = new Array(starColors.length).fill([0]).map((v, i) => new THREE.MeshBasicMaterial({ color: new THREE.Color(starColors[i]) }));
        const coords = new Array(2000).fill([0, 0, 0]).map(i => {
            let v: Vector3;
            do {
